...
$consulta = $mysqli->query("SELECT *  FROM texto_correos t WHERE t.id = 1");
$row_msg = mysqli_fetch_array($consulta);

$resultado = $mysqli->query("SELECT m.usuario_id, UPPER(CONCAT_WS(' ',u.nom, u.ape1, u.ape2)) AS cliente, m.fecha_ult_vis, m.mail, m.fec_ult_pedido, TIMESTAMPDIFF(DAY, fec_ult_pedido, CURDATE()) AS tiempo_ausencia, m.listaPrecios  FROM medicos m LEFT JOIN usuarios u ON m.usuario_id = u.id WHERE m.habilitado = 1 AND m.cliente_especial = 0 AND m.fec_ult_pedido IS NOT NULL"); 

$i=1;

while($row = mysqli_fetch_array($resultado))
{
	//$tiempo_ausencia = diferencia_en_dias($row['fec_ult_pedido'],$fecha_actual);
	$tiempo_ausencia = $row['tiempo_ausencia'];

	//echo 'Medico '.$i.' = '.$row['cliente'].' - tiempo:'.$tiempo_ausencia.' -- Ultimo pedido:'.$row['fec_ult_pedido'].'<br>';

	if (!is_null($tiempo_ausencia)) {
		/*
		$resultado2 = $mysqli->query("SELECT COUNT(p.usuario_idusuario) AS n_pedidos FROM pedidos p WHERE p.fecpedido BETWEEN DATE_SUB(NOW(), INTERVAL 61 DAY) AND NOW() AND p.usuario_idusuario = ".$row['usuario_id']." GROUP BY p.usuario_idusuario");
		...
		*/
		...
		if($row['listaPrecios'] == 1)
		{
			$nueva_lista = 1;
		}
		else{
			$nueva_lista = $row['listaPrecios'] - 1;
		}

		//REVISAMOS CUANTO TIEMPO DE AUSENCIA TIENE SIN PEDIDOS
		if($tiempo_ausencia == 61)
		{
			$enviar_mail = true;
			$actualizar = "UPDATE `medicos` SET `listaPrecios`= $nueva_lista WHERE usuario_id = ".$row['usuario_id'];
			$mysqli->query($actualizar);

		}
		elseif ($tiempo_ausencia == 91) {
			$enviar_mail = true;
			$actualizar = "UPDATE `medicos` SET `listaPrecios`= $nueva_lista WHERE usuario_id = ".$row['usuario_id'];
			$mysqli->query($actualizar);
		}
		elseif ($tiempo_ausencia == 121) {
			$enviar_mail = true;
			$actualizar = "UPDATE `medicos` SET `listaPrecios`= $nueva_lista WHERE usuario_id = ".$row['usuario_id'];
			$mysqli->query($actualizar);
		}
		elseif ($tiempo_ausencia == 151) {
			$enviar_mail = true;
			$actualizar = "UPDATE `medicos` SET `listaPrecios`= $nueva_lista WHERE usuario_id = ".$row['usuario_id'];
			$mysqli->query($actualizar);
		}
		elseif ($tiempo_ausencia > 151) {
			...
			//INHABILITAMOS EL CLIENTE
			$actualizar = "UPDATE `medicos` SET `habilitado`= 0 WHERE usuario_id = ".$row['usuario_id'];
			$mysqli->query($actualizar);
		}
		else{
			$enviar_mail = false;
			...
			//echo 'Usuario '.$i.' => '.$row['cliente'].' - ID: '.$row['usuario_id'].' --> Tiempo : '.$tiempo_ausencia.'---> Fecha visita = '.$row['fecha_ult_vis'].'<br>';
		if(!is_null($row['mail']) && $enviar_mail == true)
		{
			$to = $row['mail'];
					...
					<style type="text/css">
						...
						*/
						.templateButton{
							...
							/*@editable*/ background-color:#B0BC00;
							/*@editable*/ border:0;
			                                                    ...
			                                                    <tr>
			                                                        <td valign="top" class="bodyContent">
			                                                            <div mc:edit="std_content00">
			                                                                ...
			                                                                <h3 class="h3">Hola <b>'.$nombre.'</b> llevas '.$tiempo_ausencia.' días sin contactarnos</h3>
			                                                    ...
			                                                    <tr>
			                                                        <td valign="top">
			                                                            <div mc:edit="std_footer">
																			<em>Copyright &copy; 2017 Forza CRM, Todos los derechos reservados.</em>
																			<br />
			...
			if(mail($to,$subject,$htmlContent,$headers)){
			  	echo "0";
			  	$insertar = "INSERT INTO `email_enviados` (usuario_id, campana, fecha)VALUES(".$row["usuario_id"].", 2, CURRENT_TIMESTAMP())";
