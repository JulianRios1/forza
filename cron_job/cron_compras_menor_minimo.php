...
$consulta = $mysqli->query("SELECT *  FROM texto_correos t WHERE t.id = 2");
$row_msg = mysqli_fetch_array($consulta);


//LE RESTAMOS UN AÑO A LA FECHA SI ES ENERO EL MES ACTUAL
if(date('m') == 1)
{
	$mes_consulta = 12;
	$ano_consulta = (date('Y')-1);
}else{
	$mes_consulta = date('m');
	$ano_consulta = date('Y');
}

$resultado = $mysqli->query("SELECT CONCAT_WS(' ',u.nom,u.ape1,u.ape2) AS nombre, m.usuario_id, m.mail, ca.compra FROM cartera_usuarios ca JOIN usuarios u ON u.documento = ca.cliente_doc 
JOIN medicos m ON u.id = m.usuario_id WHERE m.habilitado = 1 AND m.cliente_especial = 0 AND u.estado = 1 AND u.idrol = 5 AND ca.ano = ".$ano_consulta." AND ca.mes = ".$mes_consulta." AND ca.compra < 216000 AND m.mail != '' ORDER BY ca.compra DESC"); 
...
while($row = mysqli_fetch_array($resultado))
{
	$nombre = (strtoupper($row['nombre']));
		...
		if(!is_null($row['mail']))
		{
			...
			$subject = utf8_decode("En ".$GLOBALS['company']." queremos apoyar tu crecimiento");
			...
			if(mail($to,$subject,$htmlContent,$headers)){
			  	...
			  	$insertar = "INSERT INTO `email_enviados` (usuario_id, campana, fecha)VALUES(".$row["usuario_id"].", 1, CURRENT_TIMESTAMP())";
